...
} from '../../../../src/features/alert-category/redux/constants';

import {
  loadMore,
  dismissLoadMoreError,
  reducer,
} from '../../../../src/features/alert-category/redux/loadMore';

const middlewares = [thunk];
const mockStore = configureMockStore(middlewares);

describe('alert-category/redux/loadMore', () => {
  afterEach(() => {
    nock.cleanAll();
  });
  ...
  it('dispatches success action when loadMore succeeds', () => {
    ...
    return store.dispatch(loadMore())
      .then(() => {
        ...
        expect(actions[0]).toHaveProperty('type', ALERT_CATEGORY_LOAD_MORE_BEGIN);
        expect(actions[1]).toHaveProperty('type', ALERT_CATEGORY_LOAD_MORE_SUCCESS);
      });
  });
  ...
  it('dispatches failure action when loadMore fails', () => {
    ...
    return store.dispatch(loadMore({ error: true }))
      .catch(() => {
        const actions = store.getActions();
        expect(actions[0]).toHaveProperty('type', ALERT_CATEGORY_LOAD_MORE_BEGIN);
        expect(actions[1]).toHaveProperty('type', ALERT_CATEGORY_LOAD_MORE_FAILURE);
  ...
  it('handles action type ALERT_CATEGORY_LOAD_MORE_BEGIN correctly', () => {
  ...
  it('handles action type ALERT_CATEGORY_LOAD_MORE_SUCCESS correctly', () => {
    const prevState = { loadMorePending: true };
    const state = reducer(
      prevState,
      { type: ALERT_CATEGORY_LOAD_MORE_SUCCESS, data: {} }
    );
    expect(state).not.toBe(prevState); // should be immutable
    expect(state.loadMorePending).toBe(false);
  });

  it('handles action type ALERT_CATEGORY_LOAD_MORE_FAILURE correctly', () => {
    const prevState = { loadMorePending: true };
    const state = reducer(
      prevState,
      { type: ALERT_CATEGORY_LOAD_MORE_FAILURE, data: { error: new Error('some error') } }
    );
    expect(state).not.toBe(prevState); // should be immutable
    expect(state.loadMorePending).toBe(false);
  ...
  it('handles action type ALERT_CATEGORY_LOAD_MORE_DISMISS_ERROR correctly', () => {
