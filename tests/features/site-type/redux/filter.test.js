...
import {
  ...
  dismissFilterError,
  reducer,
} from '../../../../src/features/site-type/redux/filter';

const middlewares = [thunk];
const mockStore = configureMockStore(middlewares);

describe('site-type/redux/filter', () => {
  afterEach(() => {
    nock.cleanAll();
  });
  ...
  it('dispatches success action when filter succeeds', () => {
    ...
    return store.dispatch(filter())
      .then(() => {
        ...
        expect(actions[0]).toHaveProperty('type', SITE_TYPE_FILTER_BEGIN);
        expect(actions[1]).toHaveProperty('type', SITE_TYPE_FILTER_SUCCESS);
      });
  });
  ...
  it('dispatches failure action when filter fails', () => {
    ...
    return store.dispatch(filter({ error: true }))
      .catch(() => {
        const actions = store.getActions();
        expect(actions[0]).toHaveProperty('type', SITE_TYPE_FILTER_BEGIN);
        expect(actions[1]).toHaveProperty('type', SITE_TYPE_FILTER_FAILURE);
  ...
  it('handles action type SITE_TYPE_FILTER_BEGIN correctly', () => {
  ...
  it('handles action type SITE_TYPE_FILTER_SUCCESS correctly', () => {
    const prevState = { filterPending: true };
    const state = reducer(
      prevState,
      { type: SITE_TYPE_FILTER_SUCCESS, data: {} }
    );
    expect(state).not.toBe(prevState); // should be immutable
    expect(state.filterPending).toBe(false);
  });

  it('handles action type SITE_TYPE_FILTER_FAILURE correctly', () => {
    const prevState = { filterPending: true };
    const state = reducer(
      prevState,
      { type: SITE_TYPE_FILTER_FAILURE, data: { error: new Error('some error') } }
    );
    expect(state).not.toBe(prevState); // should be immutable
    expect(state.filterPending).toBe(false);
  ...
  it('handles action type SITE_TYPE_FILTER_DISMISS_ERROR correctly', () => {
