...
import {
  ...
  dismissLoadOneError,
  reducer,
} from '../../../../src/features/cause/redux/loadOne';

const middlewares = [thunk];
const mockStore = configureMockStore(middlewares);

describe('cause/redux/loadOne', () => {
  afterEach(() => {
    nock.cleanAll();
  });
  ...
  it('dispatches success action when loadOne succeeds', () => {
    ...
    return store.dispatch(loadOne())
      .then(() => {
        ...
        expect(actions[0]).toHaveProperty('type', CAUSE_LOAD_ONE_BEGIN);
        expect(actions[1]).toHaveProperty('type', CAUSE_LOAD_ONE_SUCCESS);
      });
  });
  ...
  it('dispatches failure action when loadOne fails', () => {
    ...
    return store.dispatch(loadOne({ error: true }))
      .catch(() => {
        const actions = store.getActions();
        expect(actions[0]).toHaveProperty('type', CAUSE_LOAD_ONE_BEGIN);
        expect(actions[1]).toHaveProperty('type', CAUSE_LOAD_ONE_FAILURE);
  ...
  it('handles action type CAUSE_LOAD_ONE_BEGIN correctly', () => {
  ...
  it('handles action type CAUSE_LOAD_ONE_SUCCESS correctly', () => {
    const prevState = { loadOnePending: true };
    const state = reducer(
      prevState,
      { type: CAUSE_LOAD_ONE_SUCCESS, data: {} }
    );
    expect(state).not.toBe(prevState); // should be immutable
    expect(state.loadOnePending).toBe(false);
  });

  it('handles action type CAUSE_LOAD_ONE_FAILURE correctly', () => {
    const prevState = { loadOnePending: true };
    const state = reducer(
      prevState,
      { type: CAUSE_LOAD_ONE_FAILURE, data: { error: new Error('some error') } }
    );
    expect(state).not.toBe(prevState); // should be immutable
    expect(state.loadOnePending).toBe(false);
  ...
  it('handles action type CAUSE_LOAD_ONE_DISMISS_ERROR correctly', () => {
