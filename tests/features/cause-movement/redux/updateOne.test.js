...
import {
  ...
  reducer,
} from '../../../../src/features/cause-movement/redux/updateOne';

const middlewares = [thunk];
const mockStore = configureMockStore(middlewares);

describe('cause-movement/redux/updateOne', () => {
  afterEach(() => {
    nock.cleanAll();
  });
  ...
  it('dispatches success action when updateOne succeeds', () => {
    ...
    return store.dispatch(updateOne())
      .then(() => {
        ...
        expect(actions[0]).toHaveProperty('type', CAUSE_MOVEMENT_UPDATE_ONE_BEGIN);
        expect(actions[1]).toHaveProperty('type', CAUSE_MOVEMENT_UPDATE_ONE_SUCCESS);
      });
  });
  ...
  it('dispatches failure action when updateOne fails', () => {
    ...
    return store.dispatch(updateOne({ error: true }))
      .catch(() => {
        const actions = store.getActions();
        expect(actions[0]).toHaveProperty('type', CAUSE_MOVEMENT_UPDATE_ONE_BEGIN);
        expect(actions[1]).toHaveProperty('type', CAUSE_MOVEMENT_UPDATE_ONE_FAILURE);
  ...
  it('handles action type CAUSE_MOVEMENT_UPDATE_ONE_BEGIN correctly', () => {
  ...
  it('handles action type CAUSE_MOVEMENT_UPDATE_ONE_SUCCESS correctly', () => {
    const prevState = { updateOnePending: true };
    const state = reducer(
      prevState,
      { type: CAUSE_MOVEMENT_UPDATE_ONE_SUCCESS, data: {} }
    );
    expect(state).not.toBe(prevState); // should be immutable
    expect(state.updateOnePending).toBe(false);
  });

  it('handles action type CAUSE_MOVEMENT_UPDATE_ONE_FAILURE correctly', () => {
    const prevState = { updateOnePending: true };
    const state = reducer(
      prevState,
      { type: CAUSE_MOVEMENT_UPDATE_ONE_FAILURE, data: { error: new Error('some error') } }
    );
    expect(state).not.toBe(prevState); // should be immutable
    expect(state.updateOnePending).toBe(false);
  ...
  it('handles action type CAUSE_MOVEMENT_UPDATE_ONE_DISMISS_ERROR correctly', () => {
