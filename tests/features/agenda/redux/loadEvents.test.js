import configureMockStore from 'redux-mock-store';
import thunk from 'redux-thunk';
import nock from 'nock';

import {
  AGENDA_LOAD_EVENTS_BEGIN,
  AGENDA_LOAD_EVENTS_SUCCESS,
  AGENDA_LOAD_EVENTS_FAILURE,
  AGENDA_LOAD_EVENTS_DISMISS_ERROR,
} from '../../../../src/features/agenda/redux/constants';

import {
  loadEvents,
  dismissLoadEventsError,
  reducer,
} from '../../../../src/features/agenda/redux/loadEvents';

const middlewares = [thunk];
const mockStore = configureMockStore(middlewares);

describe('agenda/redux/loadEvents', () => {
  afterEach(() => {
    nock.cleanAll();
  });

  it('dispatches success action when loadEvents succeeds', () => {
    const store = mockStore({});

    return store.dispatch(loadEvents())
      .then(() => {
        const actions = store.getActions();
        expect(actions[0]).toHaveProperty('type', AGENDA_LOAD_EVENTS_BEGIN);
        expect(actions[1]).toHaveProperty('type', AGENDA_LOAD_EVENTS_SUCCESS);
      });
  });

  it('dispatches failure action when loadEvents fails', () => {
    const store = mockStore({});

    return store.dispatch(loadEvents({ error: true }))
      .catch(() => {
        const actions = store.getActions();
        expect(actions[0]).toHaveProperty('type', AGENDA_LOAD_EVENTS_BEGIN);
        expect(actions[1]).toHaveProperty('type', AGENDA_LOAD_EVENTS_FAILURE);
        expect(actions[1]).toHaveProperty('data.error', expect.anything());
      });
  });

  it('returns correct action by dismissLoadEventsError', () => {
    const expectedAction = {
      type: AGENDA_LOAD_EVENTS_DISMISS_ERROR,
    };
    expect(dismissLoadEventsError()).toEqual(expectedAction);
  });

  it('handles action type AGENDA_LOAD_EVENTS_BEGIN correctly', () => {
    const prevState = { loadEventsPending: false };
    const state = reducer(
      prevState,
      { type: AGENDA_LOAD_EVENTS_BEGIN }
    );
    expect(state).not.toBe(prevState); // should be immutable
    expect(state.loadEventsPending).toBe(true);
  });

  it('handles action type AGENDA_LOAD_EVENTS_SUCCESS correctly', () => {
    const prevState = { loadEventsPending: true };
    const state = reducer(
      prevState,
      { type: AGENDA_LOAD_EVENTS_SUCCESS, data: {} }
    );
    expect(state).not.toBe(prevState); // should be immutable
    expect(state.loadEventsPending).toBe(false);
  });

  it('handles action type AGENDA_LOAD_EVENTS_FAILURE correctly', () => {
    const prevState = { loadEventsPending: true };
    const state = reducer(
      prevState,
      { type: AGENDA_LOAD_EVENTS_FAILURE, data: { error: new Error('some error') } }
    );
    expect(state).not.toBe(prevState); // should be immutable
    expect(state.loadEventsPending).toBe(false);
    expect(state.loadEventsError).toEqual(expect.anything());
  });

  it('handles action type AGENDA_LOAD_EVENTS_DISMISS_ERROR correctly', () => {
    const prevState = { loadEventsError: new Error('some error') };
    const state = reducer(
      prevState,
      { type: AGENDA_LOAD_EVENTS_DISMISS_ERROR }
    );
    expect(state).not.toBe(prevState); // should be immutable
    expect(state.loadEventsError).toBe(null);
  });
});

