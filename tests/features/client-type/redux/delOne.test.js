...
import {
  ...
  dismissDelOneError,
  reducer,
} from '../../../../src/features/client-type/redux/delOne';

const middlewares = [thunk];
const mockStore = configureMockStore(middlewares);

describe('client-type/redux/delOne', () => {
  afterEach(() => {
    nock.cleanAll();
  });
  ...
  it('dispatches success action when delOne succeeds', () => {
    ...
    return store.dispatch(delOne())
      .then(() => {
        ...
        expect(actions[0]).toHaveProperty('type', CLIENT_TYPE_DEL_ONE_BEGIN);
        expect(actions[1]).toHaveProperty('type', CLIENT_TYPE_DEL_ONE_SUCCESS);
      });
  });
  ...
  it('dispatches failure action when delOne fails', () => {
    ...
    return store.dispatch(delOne({ error: true }))
      .catch(() => {
        const actions = store.getActions();
        expect(actions[0]).toHaveProperty('type', CLIENT_TYPE_DEL_ONE_BEGIN);
        expect(actions[1]).toHaveProperty('type', CLIENT_TYPE_DEL_ONE_FAILURE);
  ...
  it('handles action type CLIENT_TYPE_DEL_ONE_BEGIN correctly', () => {
  ...
  it('handles action type CLIENT_TYPE_DEL_ONE_SUCCESS correctly', () => {
    const prevState = { delOnePending: true };
    const state = reducer(
      prevState,
      { type: CLIENT_TYPE_DEL_ONE_SUCCESS, data: {} }
    );
    expect(state).not.toBe(prevState); // should be immutable
    expect(state.delOnePending).toBe(false);
  });

  it('handles action type CLIENT_TYPE_DEL_ONE_FAILURE correctly', () => {
    const prevState = { delOnePending: true };
    const state = reducer(
      prevState,
      { type: CLIENT_TYPE_DEL_ONE_FAILURE, data: { error: new Error('some error') } }
    );
    expect(state).not.toBe(prevState); // should be immutable
    expect(state.delOnePending).toBe(false);
  ...
  it('handles action type CLIENT_TYPE_DEL_ONE_DISMISS_ERROR correctly', () => {
