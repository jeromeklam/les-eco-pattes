...
import {
  ...
  reducer,
} from '../../../../src/features/alert/redux/loadPendings';

const middlewares = [thunk];
const mockStore = configureMockStore(middlewares);

describe('alert/redux/loadPendings', () => {
  afterEach(() => {
    nock.cleanAll();
  });
  ...
  it('dispatches success action when loadPendings succeeds', () => {
    ...
    return store.dispatch(loadPendings())
      .then(() => {
        ...
        expect(actions[0]).toHaveProperty('type', ALERT_LOAD_PENDINGS_BEGIN);
        expect(actions[1]).toHaveProperty('type', ALERT_LOAD_PENDINGS_SUCCESS);
      });
  });
  ...
  it('dispatches failure action when loadPendings fails', () => {
    ...
    return store.dispatch(loadPendings({ error: true }))
      .catch(() => {
        const actions = store.getActions();
        expect(actions[0]).toHaveProperty('type', ALERT_LOAD_PENDINGS_BEGIN);
        expect(actions[1]).toHaveProperty('type', ALERT_LOAD_PENDINGS_FAILURE);
  ...
  it('handles action type ALERT_LOAD_PENDINGS_BEGIN correctly', () => {
  ...
  it('handles action type ALERT_LOAD_PENDINGS_SUCCESS correctly', () => {
    const prevState = { loadPendingsPending: true };
    const state = reducer(
      prevState,
      { type: ALERT_LOAD_PENDINGS_SUCCESS, data: {} }
    );
    expect(state).not.toBe(prevState); // should be immutable
    expect(state.loadPendingsPending).toBe(false);
  });

  it('handles action type ALERT_LOAD_PENDINGS_FAILURE correctly', () => {
    const prevState = { loadPendingsPending: true };
    const state = reducer(
      prevState,
      { type: ALERT_LOAD_PENDINGS_FAILURE, data: { error: new Error('some error') } }
    );
    expect(state).not.toBe(prevState); // should be immutable
    expect(state.loadPendingsPending).toBe(false);
  ...
  it('handles action type ALERT_LOAD_PENDINGS_DISMISS_ERROR correctly', () => {
