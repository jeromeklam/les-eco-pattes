...
export class Search extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      search: '',
      list: [],
      lines: getNewNormalizedObject('FreeAsso_Contract'),
      loading: false,
      finish: false,
      page: 1,
      size: 10,
  ...
  onSearch(filters, page = null) {
    ...
    if (!this.state.loading) {
      ...
      const dSort = [{"col":"ct_code","way":"up"}];
      let sort = '';
      dSort.forEach(elt => {
        let add = elt.col;
        if (elt.way === 'down') {
      ...
      doRequest
        .then(result => {
          let items = [];
          let lines = getNewNormalizedObject('FreeAsso_Contract');
    ...
    return (
      <SearchModal
        title={this.props.intl.formatMessage({
          id: 'app.features.contract.search.title',
