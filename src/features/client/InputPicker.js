...
export default class InputPicker extends Component {
  ...
  onChange(event) {
    ...
    if (search.length >= 2) {
      freeAssoApi
        .get(process.env.REACT_APP_BO_URL + '/v1/asso/client/autocomplete/' + search, {
          headers: headers,
        })
        .then(result => {
          if (result && result.data && result.data.data) {
            ...
            if (Array.isArray(list) && list.length === 1) {
              this.props.onChange({
                target: { name: this.props.name, value: list[0].id, type: 'FreeAsso_Client' },
              });
            } else {
    ...
    return (
      <div className=".client-picker">
        {!this.state.zoomButton ? (
          ...
          <div className=".client-button-picker">
            <DefaultButtonPicker
              label={this.props.label}
              value={this.state.value}
              display={this.state.display}
