...
export class Search extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      search: '',
      list: [],
      lines: getNewNormalizedObject('FreeAsso_Sickness'),
      loading: false,
      finish: false,
      page: 1,
      size: 10,
  ...
  onSearch(filters, page = null) {
    ...
    if (!this.state.loading) {
      ...
      const dSort = [{"col":"sick_name","way":"up"},{"col":"sick_desc","way":"up"}];
      let sort = '';
      dSort.forEach(elt => {
        let add = elt.col;
        if (elt.way === 'down') {
      ...
      doRequest
        .then(result => {
          let items = [];
          let lines = getNewNormalizedObject('FreeAsso_Sickness');
    ...
    return (
      <SearchModal
        title={this.props.intl.formatMessage({
          id: 'app.features.sickness.search.title',
