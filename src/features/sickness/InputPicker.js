...
export default class InputPicker extends Component {
  ...
  onChange(event) {
    ...
    if (search.length >= 2) {
      freeAssoApi
        .get(process.env.REACT_APP_BO_URL + '/v1/asso/sickness/autocomplete/' + search, {
          headers: headers,
        })
        .then(result => {
          if (result && result.data && result.data.data) {
            ...
            if (Array.isArray(list) && list.length === 1) {
              this.props.onChange({
                target: { name: this.props.name, value: list[0].id, type: 'FreeAsso_Sickness' },
              });
            } else {
    ...
    return (
      <div className=".sickness-picker">
        {!this.state.zoomButton ? (
          ...
          <div className=".sickness-button-picker">
            <DefaultButtonPicker
              label={this.props.label}
              labelTop={this.props.labelTop || false}
              value={this.state.value}
